...
class QuestionsProcessor:
    def __init__(
        self,
# === 修正：加上 data/test_set/ 中间层级 ===
        vector_db_dir: Union[str, Path] = '/public2/home/skyblue/RAG-Challenge-2/data/test_set/databases/vector_dbs',
        documents_dir: Union[str, Path] = '/public2/home/skyblue/RAG-Challenge-2/data/test_set/databases/chunked_reports',
        ...
        # === 核心优化：在初始化时加载一次 Retriever，而不是在每个问题里重复加载 ===
        print("Initializing Retriever (Loading models and vector DBs)...")
    ...
    def _extract_references(self, pages_list: list, company_name: str) -> list:
        ...
        # Optimization: Check if dataframe is already loaded to avoid reading CSV every time
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        # === 核心优化：直接使用 self.retriever，不再重复创建 ===
        ...
        if not retrieval_results:
            # print(f"Warning: No context found for {company_name}")
