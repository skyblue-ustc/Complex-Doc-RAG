...
class BaseOpenaiProcessor:
    ...
    def send_message(
        ...
        # Note: deepseek-chat usually supports temperature, deepseek-reasoner might not.
        if "deepseek-reasoner" in model:
             pass
        elif "deepseek-chat" not in model:
        ...
        if not is_structured:
            ...
            # === 核心修改开始：针对 DeepSeek 做特殊处理 ===
            is_deepseek = "deepseek" in model or "deepseek" in str(self.llm.base_url)
            
            if is_deepseek:
                # DeepSeek 不支持 OpenAI 的 beta.parse (Structured Outputs)
                # 所以我们使用 json_object 模式，然后在本地用 json_repair 和 Pydantic 解析
                ...
                try:
                    ...
                    print(f"DeepSeek Structured Output Error: {e}")
                    print(f"Raw Content was: {locals().get('raw_content', 'Not generated')}")
                    # 发生错误时返回空字典或抛出，这里选择返回空字典避免程序崩溃
                    content = {}
                    # 如果需要调试，可以在这里 raise e

            else:
                # 对于原生 OpenAI 模型，继续使用 Strict Mode
